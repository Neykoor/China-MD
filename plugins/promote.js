export default [
  {
    command: ["promote", "promover"],
    description: "Promueve a admin a un participante. Uso: !promote @mención o !promote número",
    adminOnly: true,
    botAdmin: true,
    async execute({ sock, msg, remoteJid, args, reply }) {
      const mentioned = msg.message?.extendedTextMessage?.contextInfo?.mentionedJid?.[0];
      const fromArg = args[0] ? args[0].replace(/\D/g, "") : null;
      const target = mentioned || (fromArg ? fromArg + "@s.whatsapp.net" : null);
      if (!target) return reply("Menciona o indica el número a promover.");
      await sock.groupParticipantsUpdate(remoteJid, [target], "promote").catch(() => {});
      await reply("✅ Participante promovido a admin.");
    },
  },
];
